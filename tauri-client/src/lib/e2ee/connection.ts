...
export class E2EEConnection {
  ...
  disconnect(): void {
    // Set connected=false FIRST so any re-entrant calls to closeWsStream/sendWsData
    // (triggered by _onClosed event handlers below) are no-ops.
    this._connected = false;
    this.dek = null;
    this.dekResolver = null;
  ...
  async remoteFetch(url: string, init?: RequestInit): Promise<Response> {
    ...
    // Decode base64 → UTF-8 string (symmetric with encode on line 262
    // and WS decode on line 531).
    const bodyBytes = decodeURIComponent(escape(atob(response.body)));
    const respHeaders = new Headers();
    for (const [k, v] of response.headers) {
      respHeaders.set(k, v);
    }
    // Ensure Content-Type is set so Response.json() knows to parse as JSON
  ...
  private handleMessage(
    ...
    switch (msg.type) {
      ...
      case 'machine_offline':
        ...
        this.machineListeners.forEach((cb) => cb(this._machines));
        // Bridge disconnected — invalidate DEK
