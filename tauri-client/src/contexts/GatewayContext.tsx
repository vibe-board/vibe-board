 ...
 *
 * Mobile adaptation: uses gateway URL from connection store and
 * @tauri-apps/plugin-http for auth API calls.
 */
import {
  createContext,
  useContext,
...
export function GatewayProvider({ children }: { children: ReactNode }) {
  const gatewayUrl = useConnectionStore((s) => s.serverUrl);
  ...
  // Step 1: Detect gateway mode (desktop) or skip straight to session check (mobile)
  useEffect(() => {
    // On mobile, gateway is the only supported mode — skip detection
    if (isMobile()) {
      enterGatewayFlow();
      return;
    }
    detectGatewayMode(gatewayUrl).then((isGateway) => {
      if (!isGateway) {
        setPhase('local');
        return;
      }
  ...
  function enterGatewayFlow() {
    ...
    fetch(`${baseUrl}/api/auth/registration-status`, { method: 'GET' })
      .then((r) => r.json())
  ...
  const connectToMachine = useCallback(
    ...
    [session, connectionInstance, gatewayUrl, manager]
  );
...
export function useGateway(): GatewayContextValue {
  ...
  if (!ctx) {
    // Return a safe default when used outside GatewayProvider (local mode)
    return {
      phase: 'local',
      gatewayUrl: '',
      session: null,
