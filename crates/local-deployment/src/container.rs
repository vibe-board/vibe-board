...
async fn repo_worktree_path(
    ...
    if workspace.mode == WorkspaceMode::Direct && repos.len() == 1 {
        ...
        match WorkspaceRepo::worktree_subdir(pool, workspace.id, repo).await {
            Ok(subdir) => workspace_root.join(subdir),
            // On lookup failure, fall back to the flat layout.
            Err(_) => workspace_root.join(&repo.name),
        }
    }
}
...
impl LocalContainerService {
    ...
    pub async fn cleanup_workspace(db: &DBService, workspace: &Workspace) {
        ...
        if workspace.mode == WorkspaceMode::Direct && repositories.len() == 1 {
            ...
            let subdirs = WorkspaceRepo::worktree_subdirs_for_workspace(&db.pool, workspace.id)
                .await
                .unwrap_or_default();
            WorkspaceManager::cleanup_workspace(&workspace_dir, &repositories, &subdirs)
                .await
                .unwrap_or_else(|e| {
                    tracing::warn!(
    ...
    async fn update_after_head_commits(&self, exec_id: Uuid) {
        if let Ok(ctx) = ExecutionProcess::load_context(&self.db.pool, exec_id).await {
            ...
            for repo in &ctx.repos {
                let repo_path = repo_worktree_path(
                    &self.db.pool,
                    &workspace_root,
                    &ctx.workspace,
                    &ctx.repos,
    ...
    async fn check_repos_for_changes(
        &self,
        workspace_root: &Path,
        workspace: &Workspace,
        repos: &[Repo],
        ...
        for repo in repos {
            let worktree_path =
                repo_worktree_path(&self.db.pool, workspace_root, workspace, repos, repo).await;
    ...
    async fn copy_files_and_images(
        ...
        for repo in &repos {
            ...
            {
                let subdir = WorkspaceRepo::worktree_subdir_by_repo_id(
                    &self.db.pool,
                    workspace.id,
                    repo.id,
                    &repo.name,
    ...
    /// Update EP completion AND immediately push patch to events stream.
    /// Always use this instead of raw ExecutionProcess::update_completion
    /// to ensure the frontend sees status changes without relying on hooks.
    async fn update_completion_and_push(
        &self,
        ep_id: Uuid,
        status: ExecutionProcessStatus,
        exit_code: Option<i64>,
    ) -> Result<(), sqlx::Error> {
        ExecutionProcess::update_completion(&self.db.pool, ep_id, status, exit_code).await?;
        if let Ok(Some(ep)) = ExecutionProcess::find_by_id(&self.db.pool, ep_id).await {
            self.events
                .push_patch(execution_process_patch::replace(&ep));
        }
        Ok(())
    }

    /// Update task status AND immediately push patch to events stream.
    async fn update_task_status_and_push(
        &self,
        task_id: Uuid,
        status: TaskStatus,
    ) -> Result<(), sqlx::Error> {
        Task::update_status(&self.db.pool, task_id, status).await?;
        if let Ok(Some(task)) = Task::find_by_id(&self.db.pool, task_id).await {
            self.events.push_patch(task_patch::replace(&task));
        }
        Ok(())
    }
...
impl ContainerService for LocalContainerService {
    ...
    async fn is_container_clean(&self, workspace: &Workspace) -> Result<bool, ContainerError> {
        ...
        for repo in &repositories {
            let worktree_path = repo_worktree_path(
                &self.db.pool,
                &workspace_dir,
                workspace,
                &repositories,
    ...
    async fn stop_execution(
        ...
        if let Some(cancel) = self.take_cancellation_token(&execution_process.id).await {
            ...
            {
                match tokio::time::timeout(Duration::from_secs(2), monitor_handle).await {
                    Ok(_) => {
        ...
        if let Ok(ctx) = ExecutionProcess::load_context(&self.db.pool, execution_process.id).await
            ...
            && let Err(e) = self
                .update_task_status_and_push(ctx.task.id, TaskStatus::InReview)
                .await
        {
    ...
    async fn try_commit_changes(&self, ctx: &ExecutionContext) -> Result<bool, ContainerError> {
        ...
        let repos_with_changes = self
            .check_repos_for_changes(&workspace_root, &ctx.workspace, &ctx.repos)
            .await?;
