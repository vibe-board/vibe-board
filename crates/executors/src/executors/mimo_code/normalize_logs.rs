    ...
    tokio::spawn(async move {
        ...
        while let Some(Ok(line)) = stdout_lines.next().await {
            ...
            match event {
                ...
                MiMoCodeExecutorEvent::SlashCommandResult { message } => {
                ...
                MiMoCodeExecutorEvent::ApprovalRequested {
                    tool_call_id,
                    approval_id,
                }
                | MiMoCodeExecutorEvent::QuestionAsked {
                    tool_call_id,
                    approval_id,
                } => {
                ...
                MiMoCodeExecutorEvent::ApprovalResponse {
                    tool_call_id,
                    status,
                } => {
                ...
                MiMoCodeExecutorEvent::QuestionResponse {
                    tool_call_id,
                    status,
                } => {
                ...
                MiMoCodeExecutorEvent::SystemMessage { content } => {
                    let idx = entry_index.next();
                ...
                MiMoCodeExecutorEvent::Error { message } => {
                    let idx = entry_index.next();
                    msg_store.push_patch(
                        crate::logs::utils::ConversationPatch::add_normalized_entry(
                            idx,
                ...
                MiMoCodeExecutorEvent::Done => {}
            }
        }
    });
}

fn parse_event(line: &str) -> Option<MiMoCodeExecutorEvent> {
    serde_json::from_str::<MiMoCodeExecutorEvent>(line.trim()).ok()
}
    ...
    fn handle_sdk_event(
        ...
        let Some(event) = SdkEvent::parse(raw) else {
            ...
            if !raw_text.trim().is_empty() {
                ...
                // These come from other agents using the ACP protocol, not MiMoCode's SDK
                let is_acp_event = raw.get("type").is_some() && raw.get("event").is_none();
                if !is_acp_event {
                    self.add_normalized_entry(system_message(format!(
                        "Unrecognized MiMoCode SDK event: {raw_text}"
                    )));
                }
            }
            return;
            ...
            SdkEvent::SessionError(event) => {
                let (error_type, message) = match event.error {
                    Some(err) if err.kind() == "ProviderAuthError" => (
                        ...
                        err.message()
                            .unwrap_or_else(|| format!("MiMoCode session error: {}", err.raw)),
                    ),
                    Some(err) => (
                        NormalizedEntryError::Other,
                        format!("MiMoCode session error: {}", err.raw),
                    ),
                    None => (
                        NormalizedEntryError::Other,
                        "MiMoCode session error".to_string(),
                    ),
                };
