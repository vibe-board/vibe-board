...
use crate::{
    ...
    executors::{
        ExecutorError,
        mimo_code::{MiMoCodeServer, models::maybe_emit_token_usage},
    },
};
...
impl LogWriter {
    ...
    pub async fn log_event(&self, event: &MiMoCodeExecutorEvent) -> Result<(), ExecutorError> {
        let raw =
    ...
    pub async fn log_error(&self, message: String) -> Result<(), ExecutorError> {
        self.log_event(&MiMoCodeExecutorEvent::Error { message })
            .await
    }

    pub async fn log_slash_command_result(&self, message: String) -> Result<(), ExecutorError> {
        self.log_event(&MiMoCodeExecutorEvent::SlashCommandResult { message })
            .await
    }
...
/// Generate a cryptographically secure random password for MiMoCode server auth.
...
pub async fn run_session(
    ...
    let client = build_mimocode_client(&config.directory, &config.server_password)?;

    run_session_inner(config, log_writer, client, cancel).await
}

pub(super) async fn discover_commands(
    server: &MiMoCodeServer,
    directory: &Path,
) -> Result<Vec<CommandInfo>, ExecutorError> {
    let directory = directory.to_string_lossy();
    let client = build_mimocode_client(&directory, &server.server_password)?;
...
pub async fn run_slash_command(
    config: RunConfig,
    log_writer: LogWriter,
    command: slash_commands::MiMoCodeSlashCommand,
    cancel: CancellationToken,
) -> Result<(), ExecutorError> {
    let client = build_mimocode_client(&config.directory, &config.server_password)?;
...
async fn run_session_inner(
    ...
    log_writer
        .log_event(&MiMoCodeExecutorEvent::SessionStart {
        ...
        && !status.is_empty()
    {
        ...
        tracing::debug!("Sending commit reminder prompt to MiMoCode session");

        // Log as system message so it's visible in the UI (user_message gets filtered out)
        let _ = log_writer
            .log_event(&MiMoCodeExecutorEvent::SystemMessage {
    ...
    log_writer.log_event(&MiMoCodeExecutorEvent::Done).await?;
...
fn build_default_headers(directory: &str, password: &str) -> HeaderMap {
    ...
    if let Ok(value) = HeaderValue::from_str(directory) {
        headers.insert("x-mimocode-directory", value);
    }
    let credentials = BASE64.encode(format!("mimocode:{password}"));
...
pub fn build_mimocode_client(
    ...
    const MIMOCODE_HTTP_TIMEOUT: Duration = Duration::from_secs(30);
    const MIMOCODE_CONNECT_TIMEOUT: Duration = Duration::from_secs(5);

    reqwest::Client::builder()
        .default_headers(build_default_headers(directory, password))
        .connect_timeout(MIMOCODE_CONNECT_TIMEOUT)
        .timeout(MIMOCODE_HTTP_TIMEOUT)
        .build()
        ...
        tokio::select! {
            ...
            event = control_rx.recv() => match event {
                ...
                Some(ControlEvent::Disconnected) if !cancel.is_cancelled() => {
                    return Err(ExecutorError::Io(io::Error::other("MiMoCode event stream disconnected while request was running")));
                }
    ...
    loop {
        if tokio::time::Instant::now() > deadline {
            return Err(ExecutorError::Io(io::Error::other(format!(
                "Timed out waiting for MiMoCode server health: {}",
...
pub async fn create_session(
    ...
    if !resp.status().is_success() {
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.create failed: HTTP {}",
            resp.status()
        ))));
    }
...
pub async fn fork_session(
    ...
    if !resp.status().is_success() {
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.fork failed: HTTP {}",
            resp.status()
        ))));
    }
...
async fn prompt(
    ...
    let resp = client
        ...
        .query(&[("directory", directory)])
        .timeout(MIMOCODE_PROMPT_TIMEOUT)
        .json(&req)
        .send()
        .await
        .map_err(|err| ExecutorError::Io(io::Error::other(err)))?;
    ...
    // The MiMoCode server uses streaming responses and may set the HTTP status early; validate
    // success using the response body shape as well.
    if !status.is_success() {
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.prompt failed: HTTP {status} {body}"
        ))));
    }
    ...
    if trimmed.is_empty() {
        return Err(ExecutorError::Io(io::Error::other(
            "MiMoCode session.prompt returned empty response body",
        )));
    }
    ...
    if let Some(name) = parsed.get("name").and_then(Value::as_str) {
        let message = parsed
            ...
            .unwrap_or(trimmed);
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.prompt failed: {name}: {message}"
        ))));
    }

    Err(ExecutorError::Io(io::Error::other(format!(
        "MiMoCode session.prompt returned unexpected response: {trimmed}"
    ))))
}
...
pub async fn session_command(
    ...
    if !status.is_success() {
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.command failed: HTTP {status} {body}"
        ))));
    }
    ...
    if trimmed.is_empty() {
        return Err(ExecutorError::Io(io::Error::other(
            "MiMoCode session.command returned empty response body",
        )));
    }
    ...
    if let Some(name) = parsed.get("name").and_then(Value::as_str) {
        let message = parsed
            ...
            .unwrap_or(trimmed);
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode session.command failed: {name}: {message}"
        ))));
    }

    Err(ExecutorError::Io(io::Error::other(format!(
        "MiMoCode session.command returned unexpected response: {trimmed}"
    ))))
}
...
pub async fn resolve_compaction_model(
    ...
    Err(ExecutorError::Io(io::Error::other(
        "MiMoCode compaction requires a configured model",
    )))
}
...
pub async fn connect_event_stream(
    ...
    if !resp.status().is_success() {
        ...
        let body = resp
            ...
            .unwrap_or_else(|_| "<failed to read response body>".to_string());
        return Err(ExecutorError::Io(io::Error::other(format!(
            "MiMoCode event stream failed: HTTP {status} {body}"
        ))));
    }
        ...
        let current_resp = match resp.take() {
            ...
            None => {
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        let _ = log_writer
                            .log_error(format!("MiMoCode event stream reconnect failed: {err}"))
                            .await;
                        attempt += 1;
    ...
    loop {
        ...
        let Ok(data) = serde_json::from_str::<Value>(trimmed) else {
            let _ = ctx
                .log_writer
                .log_error(format!(
                    "MiMoCode event stream delivered non-JSON event payload: {trimmed}"
                ))
                .await;
            continue;
        };
        ...
        match event_type {
            ...
            "session.error" => {
                ...
                let message = data
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or("MiMoCode session error")
                    .to_string();
            ...
            "permission.asked" => {
                ...
                tokio::spawn(async move {
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            handle_approval_error(
                                err,
                                &format!(
                                    "MiMoCode approval creation failed for tool_call_id={tool_call_id}"
                                ),
                                &log_writer,
                                &tool_call_id,
                                false,
                    ...
                    let _ = log_writer
                        .log_event(&MiMoCodeExecutorEvent::ApprovalRequested {
                    ...
                    let status =
                            ...
                            .await
                        {
                            ...
                            Err(err) => {
                                ...
                                err,
                                &format!(
                                    "MiMoCode approval wait failed for tool_call_id={tool_call_id}"
                                ),
                                &log_writer,
                                &tool_call_id,
                                false,
                    ...
                    // If we reject without a message, MiMoCode treats it as a hard stop.
            ...
            "question.asked" => {
                ...
                tokio::spawn(async move {
                        ...
                        .await
                    {
                        ...
                        Err(err) => {
                            handle_approval_error(
                                err,
                                &format!(
                                    "MiMoCode question approval creation failed for tool_call_id={tool_call_id}"
                                ),
                                &log_writer,
                                &tool_call_id,
                                true,
                    ...
                    let _ = log_writer
                        .log_event(&MiMoCodeExecutorEvent::QuestionAsked {
                            tool_call_id: tool_call_id.clone(),
                            approval_id: created.approval_id.clone(),
                        })
                        .await;
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            handle_approval_error(
                                err,
                                &format!(
                                    "MiMoCode question approval wait failed for tool_call_id={tool_call_id}"
                                ),
                                &log_writer,
                                &tool_call_id,
                                true,
                    ...
                    match status {
                        QuestionStatus::Answered { answers } => {
                            let formatted = answers_to_mimocode_format(&questions, &answers);
                            let _ = client
...
async fn log_approval_response(log_writer: &LogWriter, tool_call_id: &str, status: ApprovalStatus) {
    let _ = log_writer
        .log_event(&MiMoCodeExecutorEvent::ApprovalResponse {
...
async fn log_question_response(log_writer: &LogWriter, tool_call_id: &str, status: QuestionStatus) {
    let _ = log_writer
        .log_event(&MiMoCodeExecutorEvent::QuestionResponse {
            tool_call_id: tool_call_id.to_string(),
            status,
        })
        .await;
...
fn answers_to_mimocode_format(questions: &[Value], answers: &[QuestionAnswer]) -> Vec<Vec<String>> {
    questions
        .iter()
        .map(|q| {
