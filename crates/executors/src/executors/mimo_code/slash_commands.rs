//! MiMoCode slash command parsing, execution, and result formatting.
//!
//! This module is the central place for all slash command handling in MiMoCode.
...
/// MiMoCode slash command with known variants and custom fallback.
#[derive(Debug, Clone)]
pub enum MiMoCodeSlashCommand {
    Compact,
    Commands,
    Models {
...
impl<'a> From<SlashCommandCall<'a>> for MiMoCodeSlashCommand {
...
/// Execute a slash command using the MiMoCode SDK.
pub async fn execute(
    config: RunConfig,
    command: MiMoCodeSlashCommand,
    ...
    match &command {
        ...
        MiMoCodeSlashCommand::Models { provider } => {
        ...
        MiMoCodeSlashCommand::Mcp => {
            let mcp = sdk::mcp_status(&client, &config.base_url, &config.directory).await?;
            log_result_and_done(&log_writer, format_mcp(&mcp)).await?;
            return Ok(());
        }
        // Session-dependent commands handled below
        MiMoCodeSlashCommand::Compact | MiMoCodeSlashCommand::Custom { .. } => {}
    }

    // Validate custom commands exist
    if let MiMoCodeSlashCommand::Custom { name, .. } = &command {
    ...
    let is_compact = matches!(&command, MiMoCodeSlashCommand::Compact);
    ...
    {
        MiMoCodeSlashCommand::Compact => {
            let model = compaction_model.ok_or_else(|| {
                ExecutorError::Io(io::Error::other("MiMoCode compaction model missing"))
            })?;
            Box::pin(async move {
        ...
        MiMoCodeSlashCommand::Custom { name, arguments } => Box::pin(async move {
            sdk::session_command(
                &request_client,
                &request_base_url,
                &request_directory,
