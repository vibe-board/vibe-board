...
/// Strip an optional `-fast` suffix from the model name.
///
/// Returns the base model (if any) and whether the `-fast` suffix was present.
...
/// Build `ThreadForkParams` from a thread id plus the params that would have been
/// used to start a new thread. Mirrors the helper in upstream's codex executor.
...
const DEFAULT_CODEX_BASE: &str = "npx -y @openai/codex@0.124.0";

impl Codex {
    /// Default base command for Codex (used by setup helper; overridable via cmd.base_command_override).
        ...
        Fut: std::future::Future<Output = Result<(), ExecutorError>> + Send + 'static,
    {
        ...
        tokio::spawn(async move {
            ...
            let client = AppServerClient::new(
                ...
                approvals,
                auto_approve,
                false, // plan_mode: not wired into our fork yet
                repo_context,
                commit_reminder,
