...
impl Junie {
    fn build_command_builder(&self) -> Result<CommandBuilder, CommandBuildError> {
        let builder = CommandBuilder::new("junie").extend_params(["--acp=true"]);
        apply_overrides(builder, &self.cmd)
    }
}

#[async_trait]
impl StandardCodingAgentExecutor for Junie {
    ...
    async fn spawn(
        ...
        let harness = AcpAgentHarness::with_session_namespace("junie_sessions");
    ...
    fn default_mcp_config_path(&self) -> Option<std::path::PathBuf> {
        dirs::home_dir().map(|home| home.join(".junie").join("settings.json"))
    }
