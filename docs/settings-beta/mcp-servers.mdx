...
description: "Configure MCP (Model Context Protocol) servers to enhance your coding agents within Vibe Board with additional tools and capabilities."
---
...
<Note>
This page covers configuring MCP servers **within** Vibe Board for your coding agents. For connecting external MCP clients to Vibe Board's MCP server, see the [Vibe Board MCP Server](/integrations/vibe-board-mcp-server) guide.
</Note>

## Overview

MCP servers provide additional functionality to coding agents through standardized protocols. You can configure different MCP servers for each coding agent in Vibe Board, giving them access to specialized tools like browser automation, access to remote logs, error tracking via Sentry, or documentation from Notion.
...
Vibe Board provides one-click installation for popular MCP servers. Click a card to insert a pre-configured MCP server into the JSON configuration.

<Frame>
<img src="/images/settings-beta-popular-mcp-servers.png" alt="Popular MCP servers including Vibe Board, Context7, Playwright, Exa, Chrome DevTools, and Dev Manager" />
</Frame>
...
<Check>
These changes update the global configuration file of the coding agent and will persist even if you stop using Vibe Board.
</Check>
</Step>
</Steps>
