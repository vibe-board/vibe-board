...
Set a prefix for auto-generated branch names. When you create a new workspace or task, Vibe Board automatically creates a git branch for your changes.
...
Specify where Vibe Board stores workspace data. Workspaces are created in a `.vibe-board-workspaces` subdirectory within this path.
...
Configure how Vibe Board alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when Vibe Board runs in a background tab.
- **Push Notifications** - Receive browser notifications even when Vibe Board isn't in focus. Requires browser permission when first enabled.
