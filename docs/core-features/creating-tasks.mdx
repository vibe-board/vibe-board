...
### Retry on Executor Change

When you send a follow-up message with a different executor selected, Vibe Board automatically detects the change and creates a new task carrying over the full conversation context from the previous task. A confirmation dialog appears before the retry happens, showing which executor and variant will be used.

This is useful when you want to switch agents mid-task — for example, starting with a planning variant and switching to a coding variant, or moving from one agent to another entirely.
...
<Tip>
For detailed setup instructions and examples, see the [Vibe Board MCP Server](/integrations/vibe-board-mcp-server) documentation.
</Tip>
...
The MCP client will automatically generate structured tasks in your Vibe Board project based on this description.
