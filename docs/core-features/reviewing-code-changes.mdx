...
The commit history panel shows all commits made by the coding agent during a task attempt. Access it from the task panel to get a chronological view of what the agent committed.

- **View commit diffs**: Click any commit to see the full diff inline, showing exactly what changed in that commit.
- **Revert a commit**: Click the revert button on a specific commit to undo it. A confirmation dialog appears before the revert is applied.

<Note>
The commit history panel is separate from the main diff view. The main diff shows all changes relative to the base branch, while the commit history shows individual commits made during the task.
</Note>
