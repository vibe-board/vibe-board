...
description: "Learn how to complete tasks by rebasing, merging, and managing pull requests directly from Vibe Board"
sidebarTitle: "Completing a Task"
---

When your task is finished, Vibe Board provides integrated git operations to keep your branch up-to-date and merge your work back into the base branch.

## Continuing a Failed or Cancelled Task

When a task attempt fails or is cancelled, a **Continue** button appears on the action card in the conversation view. Click it to retry or continue execution. The Continue button preserves the executor profile and variant from the previous execution, so you don't need to reconfigure anything.

<Info>
If you need to change the executor before continuing, see the retry behaviour described in [Creating Tasks](/core-features/creating-tasks#retry-on-executor-change).
</Info>
...
When merging, Vibe Board generates a [Conventional Commits](https://www.conventionalcommits.org/)-style commit message from the diff. You can configure which agent generates this message in **Settings → General → Commit Message Agent**.
