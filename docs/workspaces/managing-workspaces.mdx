...
Worktrees are stored in `~/.vibe-board/worktrees` by default on all platforms.
...
```bash
du -sh ~/.vibe-board/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:USERPROFILE\.vibe-board\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
