...
<Info>
Agent profiles are used throughout Vibe Board wherever agents run: onboarding, default settings, attempt creation, and follow-ups.
</Info>
...
<Note>
The configuration page displays the exact file path where your settings are stored. Vibe Board saves only your overrides whilst preserving built-in defaults.
</Note>
...
<Tab title="CLINE">
  ...
  [View full documentation →](https://github.com/cline/cline)
</Tab>
...
<Tab title="KILO">
  ...
  [View full documentation →](https://github.com/Kilo-Org/kilocode)
</Tab>
</Tabs>
...
## Agent Ordering and Visibility

Customise which agents appear and in what order:

1. Go to **Settings → Agents**
2. Use the drag handle to reorder agents
3. Toggle the checkbox next to each agent to enable or disable it

<Info>
Agent ordering and visibility settings are stored in your configuration and persist across sessions. Only enabled agents appear in task creation and follow-up dialogs.
</Info>
...
<Card title="MCP Server Configuration" icon="server" href="/integrations/mcp-server-configuration">
  Configure MCP servers within Vibe Board for your coding agents
</Card>

<Card title="Vibe Board MCP Server" icon="plug" href="/integrations/vibe-board-mcp-server">
  Connect external MCP clients to Vibe Board's MCP server
</Card>
</CardGroup>
