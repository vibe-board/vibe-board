...
<Info>
If not configured, Vibe Kanban uses the task's coding agent for commit message generation.
</Info>
...
When the toggle is off, Vibe Kanban uses the built-in default prompt.

### Single-Commit Optimisation

By default, when a task branch has exactly one commit ahead of the target branch, Vibe Kanban reuses that commit's message directly instead of invoking the AI. This avoids unnecessary token usage for simple changes.
...
Enable browser push notifications for task completion, approval requests, and cancellation events. When enabled, Vibe Kanban sends desktop notifications using the browser's Notification API.
...
<Note>
Browser push notifications are particularly useful when accessing Vibe Kanban remotely via E2EE gateway, as the web UI may be open in a background tab.
</Note>
