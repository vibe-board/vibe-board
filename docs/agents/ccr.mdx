...
## Installation
CCR is available via `pnpm dlx` – no separate installation required.

```bash
pnpm dlx @musistudio/claude-code-router ui
```

This launches the CCR local UI where you configure providers and models.

## Authentication
Authenticate and configure CCR outside of Vibe Board. Follow the instructions on the CCR GitHub repo:
...
In the CCR UI (`pnpm dlx @musistudio/claude-code-router ui`):
...
## Configure Vibe Board

Vibe Board does not ship a default configuration for CCR. Add configurations to the existing Claude Code agent:
...
### Example: Claude Code agent configuration in Vibe Board

<Frame>
<img src="/images/vk-ccr-agent-config.png" alt="Claude Code agent configuration in Vibe Board" />
</Frame>

## Using CCR in Vibe Board
