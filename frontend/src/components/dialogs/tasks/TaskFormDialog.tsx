...
import { defineModal, connectionGated } from '@/lib/modals';
...
const TaskFormDialogImpl = NiceModal.create<TaskFormDialogProps>(
  connectionGated<TaskFormDialogProps>((props) => {
      ...
      try {
        if (editMode) {
          await updateTask.mutateAsync(
            {
              ...
              data: {
                ...
                image_ids:
                  images.length > 0 ? images.map((img) => img.id) : null,
              },
            },
          ...
          const shouldAutoStart =
            value.autoStart && !forceCreateOnlyRef.current;
          if (shouldAutoStart) {
