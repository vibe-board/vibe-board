...
function CommitItem({
  ...
  const [expanded, toggleExpanded] = useExpandable(`commit-body-${commit.sha}`);
  ...
  return (
    <div className="group border-b border-border/50 hover:bg-accent/50 transition-colors">
        ...
        <div className="flex-shrink-0 mt-0.5">
          {hasBody ? (
            <button
              ...
              aria-label={
                expanded ? t('commit.collapseBody') : t('commit.expandBody')
              }
            >
              {expanded ? (
      ...
      {hasBody && expanded && (
        <div className="px-3 pb-3 pl-10">
          <pre className="text-xs text-muted-foreground whitespace-pre-wrap bg-muted/50 rounded p-2 border border-border/50">
            {commit.body}
          </pre>
        </div>
      )}
